...
class FilterChromosomesPlugin:
    def input(self, inputfile):
      print( "[ " + time.strftime('%d-%b-%Y %H:%M:%S',time.localtime()) + " ]" + " Python Starting" + "" )
      self.parameters = PyIO.readParameters(inputfile)
      self.filePathForInputFile = PyPluMA.prefix()+"/"+self.parameters["inputfile"]
      self.filePathForFilterFile = PyPluMA.prefix()+"/"+self.parameters["filterfile"]
      self.ensemblDatabaseVersion = self.parameters["dbversion"]

    def run(self):
      pass

    def output(self, outputFASTAFile):
      ...
      self.filePathForFilterFile.strip()
      ...
      with open(self.filePathForFilterFile, 'r') as INFILE:
      ...
      print("[ " + time.strftime('%d-%b-%Y %H:%M:%S', time.localtime()) + " ]" + " Number of IDs: " + '{:0,.0f}'.format(numberOfLinesInFile) + "")
      ...
      for record in SeqIO.parse(self.filePathForInputFile, "fasta"):
